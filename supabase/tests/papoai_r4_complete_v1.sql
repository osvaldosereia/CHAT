-- R4 complete smoke tests — channel + multimodal
do $$
declare
  cfg jsonb;
  ready jsonb;
...
begin
  ...
  if coalesce((cfg->>'allowed_media_host_count')::integer,-1)<>0 then
    raise exception 'R4 media allowlist must remain empty before R7';
  end if;
  ...
  if coalesce((ready#>>'{image,ready}')::boolean,false) then
    raise exception 'R4 image must remain gated';
  end if;

  if coalesce((ready#>>'{inbound_audio,ready}')::boolean,false) then
    raise exception 'R4 inbound audio must remain gated';
  end if;

  if coalesce((ready#>>'{inbound_image,ready}')::boolean,false) then
    raise exception 'R4 inbound image must remain gated';
  end if;
  ...
  if r->>'resolved_type'<>'text'
    ...
    raise exception 'R4 image fallback invalid: %',r;
  end if;
  ...
  if (select enabled from public.papoai_commercial_policy_config where id=1) then
    raise exception 'commercial policy must remain disabled after R4';
  end if;
